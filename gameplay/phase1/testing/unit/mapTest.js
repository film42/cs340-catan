...
test("map.CanBuildSettlement",function(){
  ...
  ok(ports.length == modelJson.map.length, "port list size");
  var hexLoc = new catan.model.map.HexLocation(2,0);
  ...
  hexLoc = new catan.model.map.HexLocation(3,0);
  ok(!map.canBuildSettlement(0, hexLoc, "E", false), 
      "false: Ocean Vertex");
  
  hexLoc = new catan.model.map.HexLocation(4,0);
  ok(!map.canBuildSettlement(0, hexLoc, "E", false), 
...
test("Map.canBuildCity",function(){
  var map = new catan.models.map.Map(modelJson.map);
  var hexLoc = new catan.model.map.HexLocation(1,1);
...
test("Map.canBuildRoad",function(){
  ...
  var hexLoc = new catan.model.map.HexLocation(0,0);
...
test("Map.canMaritimeTrade",function(){
  var map = new catan.models.map.Map(modelJson.map);
  var resources = catan.model.ResourceList(0,0,0,2,0);
  ok(map.canMaritimeTrade(0,resources), "True: Wheat Trade");
  var resources = catan.model.ResourceList(0,0,0,3,0)
  ok(map.canMaritimeTrade(0,resources), "False: no 3:1 port");
  var resources = catan.model.ResourceList(0,0,0,0,2)
